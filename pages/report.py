...
def render(df, time_axis, metadata, sampling_rate,
    ...
    st.title("METIS Analytics PDF Report")
    ...
    if st.button("Generate PDF Report", type="primary"):
        ...
        st.download_button(
            ...
            data=pdf_bytes,
            file_name=f"metis-analytics-report_{filename}.pdf",
...
def _build_pdf(df, time_axis, metadata, sampling_rate, options: dict) -> bytes:
    ...
    def page_frame(canvas, doc):
        ...
        canvas.drawString(MARGIN + 3 * mm, PAGE_H - MARGIN - 5.5 * mm, "METIS ANALYTICS")
    ...
    if options.get('report_notes'):
        ...
        story.append(Spacer(1, 2 * mm))
    story.append(Paragraph(
        "Generated by <b>METIS Analytics</b> - independent vibration analysis tool. "
    ...
    doc = SimpleDocTemplate(
        ...
        title="METIS Analytics Measurement Report",
        author=options.get('operator', 'METIS Analytics'),
    )
...
def _build_metis_pdf(files, options: dict) -> bytes:
    import os
    from reportlab.lib.pagesizes import A4
    from reportlab.lib import colors as rl_colors
    from reportlab.lib.units import mm
    ...
    def draw_header_footer(c, metadata, page_num: int, total_pages: int):
        ...
        c.drawString(MARGIN, header_y, "METIS ANALYTICS")
        if project:
        ...
        c.drawString(MARGIN, footer_y, "METIS ANALYTICS by ABDIYASA")
    ...
    c.setTitle(f"METIS Analytics Vibration Report - {options.get('project_name') or 'Measurement'}")
    c.setAuthor(options.get("operator") or "METIS Analytics")
    c.setSubject(
        f"Vibration measurement, waveform analysis, and {compliance_heading} compliance"
    )
    c.setCreator("METIS Analytics by ABDIYASA")
