...
with st.sidebar:
    ...
    with st.expander("📁 Add Files", expanded=len(st.session_state.uploaded_files_dict) == 0):
        new_files = st.file_uploader("Upload Vibracord CSV files", type="csv", accept_multiple_files=True)
        if new_files:
            for f in new_files:
                if f.name not in st.session_state.uploaded_files_dict:
                    st.session_state.uploaded_files_dict[f.name] = f.read()
    ...
    page = st.radio("Navigate", ["📊 Data Overview", "📐 Math Analysis", "💥 Signature Hole Analysis"])
...
metadata, df, time_axis, sampling_rate = parse_file(uploaded_file)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if trunc_method == "✏️ Type start & end time manually":
        ...
        st.caption("Use the range slider below to select the waveform window.")
        t_start, t_end = st.select_slider(
            "Select time window (ms)",
            options=[round(t, 2) for t in time_axis],
            value=(time_axis[0], time_axis[-1])
        )
